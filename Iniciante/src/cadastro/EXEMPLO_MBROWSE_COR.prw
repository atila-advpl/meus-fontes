#include 'protheus.ch'
#include 'parmtype.ch'
#INCLUDE "rwmake.ch"

#DEFINE ENTER chr(13) + chr(13)

//U_EXEMPLO_MBROWSE_COR()
user function EXEMPLO_MBROWSE_COR()

	Private cCadastro := "CADASTRO DE TITULAR"

	aCores :=   { {'EMPTY(SZZ->ZZ_STATUS)', 'ENABLE'}, {'ZZ_STATUS =="I"' , 'BR_VERMELHO' },  {'ZZ_STATUS =="A"' , 'BR_VERDE'} }
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³ Monta um aRotina proprio                                           ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	Private aRotina := { {"Pesquisar","AxPesqui",0,1 } ,;
	{"Visualizar","AxVisual",0,2} ,;
	{"Incluir","AxInclui",0,3   } ,;
	{"Alterar","AxAltera",0,4   } ,;
	{"Legenda","U_Legendas()",0,5},;
	{"Excluir","AxDeleta",0,6   }}

	Private cDelFunc := ".T." // Validacao para a exclusao. Pode-se utilizar ExecBlock
	Private cString := "SZZ"
	dbSelectArea("SZZ")
	dbSetOrder(1)
	mBrowse( 6,1,22,75,cString,,,,,6,aCores)

	Return

User Function Legendas    // função para legenda colorida
	*************************
	aLegenda := {{"BR_VERMELHO","INATIVO"}, {"BR_VERDE", "ATIVO"}}
	BrwLegenda("Situação do Titular !!!","Legenda",aLegenda)
return

